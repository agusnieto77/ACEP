...
acep_sst <- function(datos, st = "mes", u = 2, d = 4) {
  if (!is.object(datos)) {
    return(message("Debe ingresar un dataframe en el par\u00e1metro datos."))
  }
  if((paste(names(datos), collapse = '') != "fechan_palabrasconflictosintensidad")){
    return(message("Debe ingresar un dataframe que contenga cuatro variables:
                   'fecha' (en formato Date), 'n_palabras' (num\u00farica),
                   'conflictos' (num\u00farica), 'intensidad' (num\u00farica).
                   Las \u00faltimas tres se pueden construir con la funci\u00f3n 'acep_db'."))
  } else {
    tryCatch({
