...
#' @description Función que devuelve un gráfico de barras con las palabras más recurrentes en un corpus textual.
#' @param x vector de palabras tokenizadas.
#' @param u numero de corte para el top de palabras más frecuentes.
#' @param frec parámetro para determinar si los valores se visualizarán como frecuencia absoluta o relativa.
#' @export acep_token_plot
#' @return Si todas las entradas son correctas, la salida será un gráfico de barras.
...
acep_token_plot <- function(x, u = 10, frec = TRUE) {
  if(frec == TRUE){
    ...
    tabla_token <- data.frame(token = tabla_token$x, frec = tabla_token$Freq, prop = tabla_token$prop)
    tabla_token <- utils::head(tabla_token, n = u)
    tabla_token <- tabla_token[order(tabla_token$frec, decreasing = FALSE),]
    graphics::barplot(height=tabla_token$frec, names=as.factor(tabla_token$token),
    ...
    tabla_token <- utils::head(tabla_token, n = u)
    tabla_token <- tabla_token[order(tabla_token$frec, decreasing = FALSE),]
    graphics::barplot(height=tabla_token$prop, names=as.factor(tabla_token$token),
